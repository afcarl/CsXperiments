"""
This code is not my property, all rights go to the respective owners.

Source: http://thirdworldnomad.com/techblog/2016/08/12/lstm-backprop/
"""
...
D = 26  # Number of input dimension == number of items in vocabulary
...
def lstm_forward(X, state):
    ...
    cache = hf, hi, ho, hc, c, c_old, h, y  # Add all intermediate variables to this cache
...
def lstm_backward(prob, y_train, d_next, cache):
    ...
    def unpack():
        ...
        hf, hi, ho, hc, c, c_old, h, y = cache
        dh_next, dc_next = d_next

        # Softmax loss gradient
        dy = prob.copy()

        return m, Wf, Wi, Wc, Wo, Wy, hf, hi, ho, hc, c, c_old, h, y, dh_next, dc_next, dy

    m, Wf, Wi, Wc, Wo, Wy, hf, hi, ho, hc, c, c_old, h, y, dh_next, dc_next, dy = unpack()
