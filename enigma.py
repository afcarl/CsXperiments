...
class Machine:
    ...
    def set_rotors(self, positions):
        if len(positions) != len(self.rotors):
            raise ValueError('Error: rotor settings do not match with number of rotors')
        else:
    ...
    def encrypt_char(self, c):
        ...
        for i, rotor in enumerate(self.rotors):
            if i is 0:
                ...
                difference = (
                             alphabet.index(self.rotors[i - 1].position) - alphabet.index(self.rotors[i].position)) % 26
